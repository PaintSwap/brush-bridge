...
const NetworkButton = ({}) => {
  ...
  return (
    <Button size="large" className={styles.mainButton} variant='contained' color="primary" onClick={() => open({ view: 'Networks' })} disabled={!isConnected} startIcon={chain?.id ? <img src={icons[chain?.id || 250]} alt={chain?.name ?? ""} width={26} height={26} /> : null} style={{whiteSpace: "nowrap"}}>
